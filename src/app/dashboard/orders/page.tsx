    ...
    <div className="p-6 sm:p-8 max-w-6xl mx-auto space-y-6">
      <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-gray-200/90 dark:border-slate-800 shadow-2xs">
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-gray-900 dark:text-slate-100 flex items-center gap-2">
          <AppIcon name={view === 'sales' ? 'orders' : 'cart'} className="w-7 h-7 text-blue-600 dark:text-blue-400" />
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-slate-900 p-12 rounded-3xl shadow-2xs text-center border border-gray-200/90 dark:border-slate-800 space-y-3 flex flex-col items-center justify-center">
          <AppIcon name="cart" className="w-12 h-12 text-gray-400 mb-1" />
          <p className="text-gray-600 dark:text-slate-300 text-sm font-bold">
            {view === 'sales' 
          ...
          <Link 
            ...
            className="bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition inline-flex items-center gap-1.5 shadow-xs"
          >
            <AppIcon name={view === 'sales' ? 'add' : 'cart'} className="w-3.5 h-3.5 text-white" />
            <span>{view === 'sales' ? 'Publicar mi primer producto' : 'Explorar Marketplace'}</span>
          </Link>
        </div>
      ) : (
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border flex items-center gap-1 ${
                      ...
                      <AppIcon name={order.status === 'completed' ? 'check' : 'pending'} className="w-3 h-3" />
                      <span>{order.status === 'completed' ? 'Completado' : order.status === 'pending' ? 'Pendiente' : order.status}</span>
                    </span>
                  </div>
                  ...
                  {order.order_items?.map((item: any, index: number) => (
                    <div key={index} className="flex justify-between items-center text-xs bg-slate-50 dark:bg-slate-800/80 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700">
                      <div className="flex items-center gap-2.5">
                        <AppIcon name="package" className="w-4 h-4 text-gray-500" />
                        <div>
